...
:- module(set, [find/2, remove/3, merge_set/3, intersect/3]).
...
/* remove_duplicate predicate removes all the duplicates from a set. 
    1. If the set is empty, then the result will be an empty set.
    2. If the head of the set is in the tail, then the result will be the tail.
*/
...
/* Difference predicate perform the difference operation between two sets (Set_1, Set_2) in a new one (Set_3).
    1. If the two sets are empty, then the difference is the emtpy set.
    2. If one of the two sets is an empty set, then the difference is the non-empty set.
    3. If the two sets are non-emtpy and the head of the first set is in the second set, 
    then the difference is the second set minus the element.
*/